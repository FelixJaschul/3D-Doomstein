...
public class Screen {
	...
	public void update(Camera camera, int[] pixels) {
		...
		for (int x = 0; x < width; x = x + 1) {
		    ...
		    if (side == 0) perpWallDistance = Math.abs((mapX - camera.xPos + (double) (1 - stepX) / 2) / raycastDirectionX);
		    ...
		    if (side==1) wallX = (camera.xPos + ((mapY - camera.yPos + (double) (1 - stepY) / 2) / raycastDirectionY) * raycastDirectionX);
		    else wallX = (camera.yPos + ((mapX - camera.xPos + (double) (1 - stepX) / 2) / raycastDirectionX) * raycastDirectionY); // X-axis wall
		    ...
		    for (int y=drawStart; y<drawEnd; y++) {
		    	...
		    	if (side==0) color = textures.get(textureNumber).pixels[texX + (texY * textures.get(textureNumber).SIZE)];
		    	else color = (textures.get(textureNumber).pixels[texX + (texY * textures.get(textureNumber).SIZE)]>>1) & 8355711; // Make y sides darker
